...
**Bonus**:
- Tailwind CSS used throughout the UI.
- CI/CD pipeline (`.github/workflows/ci-cd.yml`) runs both test suites,
  builds the frontend, and deploys `frontend/dist` to GitHub Pages.
...
npm install
npm run seed             # loads the sample employees + projects
...
### 5. Production build (single Node service, like the backend serving the built frontend)
...
- **Backend**: deploy `backend/` to any Node host (Render, Railway, Fly.io)
  with `DATABASE_URL` set as an environment variable.
- **Frontend**: the included GitHub Actions workflow builds and deploys
  `frontend/dist` to GitHub Pages automatically. Set an Actions variable
  named `VITE_API_BASE_URL` pointing at your deployed backend's `/api`
  URL so the built frontend knows where to send requests (GitHub Pages
  only serves static files, so the API must be hosted separately).
